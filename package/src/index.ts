        ...
        if (doesItemHaveChildren === true) {
          ...
          if (dataComponent) {
            ...
            if (noMoreChildrenWithAttributeDataComponent === undefined) {
              let key: string = dataComponent;
              let obj: KeyValue = {};
              obj[key] = component.toString();
              componentsArr.push(obj);
            }

            const thereIsMoreChildrenWithAttributeDataComponent =
              component.attr("data-component") === undefined ? false : true;
            ...
            if (thereIsMoreChildrenWithAttributeDataComponent === true) {
              ...
              const subComponentHtml = $(
                `div[data-component="${subComponentName}"]`
              ).toString();

              let allSubComponents: KeyValue[] = [];
              ...
              if (siblings != undefined) {
                const foundSibling = $(
                  `div[data-component="${siblings}"]`
                ).toString();

                let key1: string = subComponentName!;
                let obj1: KeyValue = {};

                obj1[key1] = subComponentHtml;

                let key2: string = siblings!;
                let obj2: KeyValue = {};
              ...
              allSubComponents.map((item) => {
                ...
                const exists = componentsArr.find(
                ...
                if (exists != undefined) {
                  ...
                  const index = componentsArr.findIndex(
                    (x) => Object.keys(x)[0] === b
                  );
